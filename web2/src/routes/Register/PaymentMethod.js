...
const PaymentMethod = ({ stripe }) => {
  ...
  const valid = useMemo(() => email.valid && firstname.valid && lastname.valid && phone.valid, [email, firstname, lastname, phone]);
  ...
  return (
    <React.Fragment>
      ...
      <CardElement/>
      <Button onClick={addPaymentMethod} disabled={loading || !valid}>Add</Button>
    </React.Fragment>
  );
};

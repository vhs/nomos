    ...
    function ($stateProvider) {
        $stateProvider.state('user.profile', {
            ...
            controller: [
                ...
                'PinService1',
                'UserService1',
                function ($scope, $timeout, PinService1, UserService1) {
                    $scope.showPin = false;
                    $scope.togglePin = function (val) {
                        $scope.showPin = val;
                    };
                    ...
                    $scope.filterLinked = function (key) {
                        return key.type === 'github' || key.type === 'slack' || key.type === 'google';
                    };
                    ...
                    $scope.updateProfile = function () {
                        ...
                        UserService1.UpdateUsername($scope.currentUser.id, $scope.profile.username).then(function () {
                            if ($scope.currentUser.hasPrivilege('full-profile')) {
                                $scope.pendingUpdate += 1;
                                UserService1.UpdateName($scope.currentUser.id, $scope.profile.fname, $scope.profile.lname).then(function () {
                                    $scope.pendingUpdate -= 1;
                                });

                                $scope.pendingUpdate += 1;
                                UserService1.UpdateEmail($scope.currentUser.id, $scope.profile.email).then(function () {
                                    $scope.pendingUpdate -= 1;
                                });

                                $scope.pendingUpdate += 1;
                                UserService1.UpdatePaymentEmail($scope.currentUser.id, $scope.profile.payment_email).then(function () {
                                    $scope.pendingUpdate -= 1;
                                });

                                $scope.pendingUpdate += 1;
                                UserService1.UpdateStripeEmail($scope.currentUser.id, $scope.profile.stripe_email).then(function () {
                                    $scope.pendingUpdate -= 1;
                                });
                            }

                            $scope.pendingUpdate += 1;
                            UserService1.UpdateNewsletter($scope.currentUser.id, $scope.profile.newsletter).then(function () {
                                $scope.pendingUpdate -= 1;
                            });

...
class StripeEventMonitor extends Monitor {
    ...
    public function handleCreated($args) {
        ...
        if ($stripe_event->status == 'VALID') {
            ...
            $full_name = !is_null($stripe_data->data->object->customer_name)
                ? $this->split_name($stripe_data->data->object->customer_name)
                : [$stripe_data->data->object->customer_email, $stripe_data->data->object->customer_email];

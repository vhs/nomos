...
export default class PinService2 implements IPinService2 {
    ...
    async GenerateTemporaryPin(expires: string, privileges: string, notes: string): BackendResult<Key> {
        return await backendCall('/services/v2/PinService2.svc/GenerateTemporaryPin', {
            expires,
            privileges,
            notes
        })
     ...
     * @returns {Key}
     */
    async GetUserPin(userid: number): BackendResult<Key> {
        return await backendCall('/services/v2/PinService2.svc/GetUserPin', {
            userid
        })
    }
